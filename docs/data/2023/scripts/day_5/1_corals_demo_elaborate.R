...
# What is happening in the conditional
# step by step
# during iteration: the first value of the vector 
# 1:length(required) is assigned to i
i <- 1
# this expression is evaluated (single logical value)
!(required[i]%in%rownames(installed))

# same as this
!("divDyn"%in%rownames(installed))

# is any of the values "divDyn"? 
any("divDyn" == rownames(installed))

# are all the values "divDyn"? 
all("divDyn" == rownames(installed))
...
# DO NOT USE THESE IN PUBLICATIONS
...
# reading in the binary R file
newRead <- readRDS(file="corals_export/divDyn_original.rds")
...
# without borders and main title
...
############################
# seleting stage-specific set of data, plotting it on a map
# interactive  running of the future function
# variables are assigned globally, so the individual lines work
x <- corals
stage <- 81
ts <- stages
map <- ne$geometry
...
plot_occs_from_stage(x=corals, stage=80)	

# a different one - no matter that x and stage have global bindings
x11()
plot_occs_from_stage(x=corals, stage=70)	
...
# all the stages at the same time
pdf("corals_export/all_stages.pdf", width=20, height=10)

  # all the occuring stg values in the table
  plottedStages <- sort(unique(corals$stg))

  for(i in plottedStages){
	plot_occs_from_stage(x=corals, stage=i)
  }
